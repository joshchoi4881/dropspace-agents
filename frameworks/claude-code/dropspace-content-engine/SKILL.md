...
```bash
export DROPSPACE_API_KEY_MYAPP="ds_live_..."   # from dropspace.dev/settings/api
export ANTHROPIC_API_KEY="sk-ant-..."           # from console.anthropic.com
export FAL_KEY="fal_..."                        # from fal.ai (for visual/video formats)
```

Save these in `load-env.sh` (copy from `templates/load-env.example.sh`) so you can `source load-env.sh` before each run.
...
This creates `~/dropspace/apps/myapp/` with config files and per-platform directories.

### 4. Edit app.json

Open `~/dropspace/apps/myapp/app.json` and fill in:
...
5. Writes strategy notes to `~/dropspace/apps/myapp/` (persists between runs)
...
0  0 * * * cd /path/to/dropspace-agents && source load-env.sh && node scripts/refresh-tracking.js --app myapp && node scripts/cleanup-posts.js --app myapp --days 7 >> /tmp/dropspace-cron.log 2>&1
30 0 * * * cd /path/to/dropspace-agents && source load-env.sh && node scripts/run-x-research.js --app myapp >> /tmp/dropspace-cron.log 2>&1
0  1 * * * cd /path/to/dropspace-agents && source load-env.sh && node scripts/run-self-improve-all.js --app myapp >> /tmp/dropspace-cron.log 2>&1
0  2 * * * cd /path/to/dropspace-agents && source load-env.sh && node scripts/schedule-day.js --app myapp >> /tmp/dropspace-cron.log 2>&1
```
...
- Check logs at `~/dropspace/apps/myapp/reports/`

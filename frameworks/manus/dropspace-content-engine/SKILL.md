...
- `DROPSPACE_API_KEY_MYAPP` — from [dropspace.dev/settings/api](https://www.dropspace.dev/settings/api)
...
5. Writes strategy notes to `~/dropspace/apps/myapp/` (persists in sandbox)
...
- **Sandbox reset:** If the pipeline doesn't run for 7 days (free) or 21 days (paid), the sandbox resets. You lose `~/dropspace/apps/` state (strategy notes, experiment tracking). Re-run `init-app.js` to recover — analytics history lives in Dropspace, not local files.

---
name: dropspace
description: Create and publish social media launches via the Dropspace API, and manage the autonomous content automation pipeline. Use when user wants to post to social media, create launches, manage personas, generate media, track post analytics, schedule content, run self-improve, engage on X, or anything Dropspace-related. Also use for the content pipeline (scheduling, self-improve, midnight reports, X research, X engagement).
---
...
Auth header: `-H "Authorization: Bearer $DROPSPACE_API_KEY_DROPSPACE"`
...
curl -s https://api.dropspace.dev/connections \
  -H "Authorization: Bearer $DROPSPACE_API_KEY_DROPSPACE"
...
**Code:** the `dropspace-agents` repo
**Data:** `~/dropspace/apps/{app}/` (created by `init-app.js`)
**Env:** `source load-env.sh`
...
- Core: `core/`
- Engage: `engage/`
- Docs: `docs/` (CRON_RULES.md, ANTI-PATTERNS.md)
...
node scripts/run-self-improve-all.js --days 14
node scripts/midnight-report.js --app dropspace --days 7
node scripts/run-x-research.js --app dropspace
node engage/engage-x.js --app dropspace --max-replies 5
echo '{"posts":[...]}' | node scripts/add-posts.js --app dropspace --platform twitter
```

For full docs, read `docs/` and `platforms/*/SKILL.md`.
